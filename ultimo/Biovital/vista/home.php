...
.password-toggle                { background: #f0f0f0; border: none; border-radius: 0 10px 10px 0; padding: 12px 15px; cursor: pointer; color: #6c757d; transition: color 0.3s; }
...
<div class="register-link" id="registerLink"></div>
<div class="register-link mt-2">
    <a href="#" id="recoverButton">¿Olvidaste tu contraseña?</a>
</div>
...
$(document).ready(function () {
    ...
    function abrirModalConRol(rol) {
        ...
        // Solo paciente puede registrarse — médico y roles admin NO muestran link de registro
        if (rol === 'paciente') {
            $('#registerLink').html('<a href="' + APP_URL + '/registro/paciente">¿No tienes cuenta? Regístrate aquí</a>').show();
        } else {
            $('#registerLink').html('<small class="text-muted"><i class="fa-solid fa-circle-info me-1"></i>Cuentas protegidas. Contacta al administrador.</small>').show();
        }
    ...
    // Abrir modal desde parámetro URL
    ...
    $('#togglePassword').on('click', function () {
        ...
        $('#eyeIcon').toggleClass('fa-eye', !isPass).toggleClass('fa-eye-slash', isPass);
    });
    ...
    $('#loginForm').on('submit', function (e) {
        ...
        $.ajax({
            ...
            timeout:  15000,
            success: function (r) {
                var inner = (r.data !== undefined) ? r.data : r;
                if (inner.success) {
                    $btn.html('<i class="fas fa-check-circle"></i> ¡Acceso concedido!');
                    window.location.href = APP_URL + '/' + inner.redirect;
                } else {
                    $('#loginError').text(inner.error || r.message || 'Cédula o contraseña incorrecta').fadeIn();
    ...
    $('.specialty-card').on('click', function () {
        ...
        $('#specialtyModalBody').html(
            '<p>' + info.d + '</p><h6 class="mt-3 mb-2">Servicios:</h6><ul>' +
            info.s.map(function (s) { return '<li>' + s + '</li>'; }).join('') + '</ul>'
        );
        ...
        entries.forEach(function (e) {
            if (e.isIntersecting && !counted) {
                ...
                $('.stat-number[data-count]').each(function () {
                    var $el = $(this), t = parseInt($el.data('count')), c = 0;
