...
class AdministradorController {    
    ...
    try {
        ...
        // Total de usuarios (todas las tablas)
        $sql = "SELECT 
                    (SELECT COUNT(*) FROM registro_paciente) as total_pacientes,
                    (SELECT COUNT(*) FROM registro_medico) as total_medicos,
                    (SELECT COUNT(*) FROM registro_asistente) as total_asistentes,
                    (SELECT COUNT(*) FROM registro_administrador) as total_administradores";
        $query = $pdo->prepare($sql);
        $query->execute();
        $totales = $query->fetch(PDO::FETCH_OBJ);
        
        $total_usuarios = ($totales->total_pacientes ?? 0) + 
                          ($totales->total_medicos ?? 0) + 
                          ($totales->total_asistentes ?? 0) + 
                          ($totales->total_administradores ?? 0);
        ...
        $query = $pdo->prepare($sql);
        $query->execute();
        $total_consultorios = $query->fetch(PDO::FETCH_OBJ)->total ?? 0;
        
        // Total de especialidades activas
        $sql = "SELECT COUNT(*) as total FROM especialidades WHERE activo = 1";
        $query = $pdo->prepare($sql);
        $query->execute();
        $total_especialidades = $query->fetch(PDO::FETCH_OBJ)->total ?? 0;
        
        // Total de citas
        $sql = "SELECT COUNT(*) as total FROM citas";
        $query = $pdo->prepare($sql);
        $query->execute();
        $total_citas = $query->fetch(PDO::FETCH_OBJ)->total ?? 0;
        
        // Citas pendientes
        $sql = "SELECT COUNT(*) as total FROM citas WHERE estado = 'pendiente' OR estado = 'confirmada'";
        $query = $pdo->prepare($sql);
        $query->execute();
        $citas_pendientes = $query->fetch(PDO::FETCH_OBJ)->total ?? 0;
        
        $stats = [
            'total_usuarios' => $total_usuarios,
            'total_pacientes' => $totales->total_pacientes ?? 0,
            'total_medicos' => $totales->total_medicos ?? 0,
            'total_asistentes' => $totales->total_asistentes ?? 0,
            'total_administradores' => $totales->total_administradores ?? 0,
            ...
            'medicos_activos' => $totales->total_medicos ?? 0,
            'consultorios_activos' => $total_consultorios,
            'especialidades_activas' => $total_especialidades,
            'pacientes_atendidos_mes' => $recetas_mes,
            'usuarios_nuevos_mes' => $recetas_mes,
            'usuarios_activos_hoy' => 0
        ];
        
        ApiResponse::success($stats, 'estadisticas', 'Estadísticas cargadas correctamente');
        
    } catch(PDOException $e) {
        error_log("Error en estadisticasGenerales: " . $e->getMessage());
        ApiResponse::error('Error al cargar estadísticas: ' . $e->getMessage(), 'server_error', [], 500);
    }
}
    // ==================== VISTAS (Rutas no-API) ====================   
