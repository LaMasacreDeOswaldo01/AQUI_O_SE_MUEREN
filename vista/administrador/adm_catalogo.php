...
$id_administrador = $_SESSION['usuario'] ?? 0;
?>
<!-- Welcome Banner -->
...
$(document).ready(function() {
    ...
    function cargarEstadisticas() {
        $.ajax({
            url: APP_URL + '/api/administradores/estadisticas-generales',
            type: 'POST',
            dataType: 'json',
            timeout: 15000,
            success: function(response) {
                ...
                var ahora = new Date();
                $('#fecha_actualizacion').text(ahora.toLocaleDateString('es-ES') + ' ' + ahora.toLocaleTimeString('es-ES'));
            ...
            error: function(xhr, status, error) {
                ...
                // Intentar cargar desde URLs alternativas
                cargarEstadisticasAlternativas();
            }
        });
    }
    
    function cargarEstadisticasAlternativas() {
        console.log('Intentando cargar estadísticas desde fuentes alternativas...');
        
        // Cargar total de consultorios
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#total_consultorios').text(data.total_consultorios || 0);
                $('#consultorios_activos').text(data.activos || 0);
            },
            error: function() { console.log('Error cargando consultorios'); }
        });
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success && response.data) data = response.data;
                $('#total_especialidades').text(data.total_especialidades || 0);
                $('#especialidades_activas').text(data.activas || 0);
            },
            error: function() { console.log('Error cargando especialidades'); }
        });
        ...
        $.ajax({
            ...
            error: function() { console.log('Error cargando recetas'); }
        });
        
        // Cargar total de médicos y pacientes desde estadísticas de usuario
        $.ajax({
            ...
            success: function(response) {
                var usuarios = [];
                if (response.success && response.data) usuarios = response.data;
                else if (Array.isArray(response)) usuarios = response;
                
                var medicos = usuarios.filter(u => u.tipo === 'medico').length;
                var pacientes = usuarios.filter(u => u.tipo === 'paciente').length;
                
                $('#total_medicos').text(medicos);
                $('#total_pacientes').text(pacientes);
                $('#total_usuarios').text(usuarios.length);
            },
            error: function() { console.log('Error cargando usuarios'); }
        });
    ...
    function cargarActividadReciente() {
        $.ajax({
            url: APP_URL + '/api/administradores/actividad-reciente',
            type: 'POST',
            dataType: 'json',
            timeout: 10000,
            success: function(response) {
